...
/// UTF-16 Little Endian encoding marker.
...
impl Encoding for Utf16Le {
    ...
    fn decode_char_before(bytes: &[u8], offset: usize) -> Option<(char, usize)> {
        ...
        let prev_unit = u16::from_le_bytes([bytes[offset - 2], bytes[offset - 1]]);
        ...
        if (SURROGATE_LOW_START..=SURROGATE_LOW_END).contains(&prev_unit) {
            ...
            let high_unit = u16::from_le_bytes([bytes[offset - 4], bytes[offset - 3]]);
...
impl crate::encoding::UniversalEncoding for Utf16Le {}
...
inventory::submit! {
    crate::registry::EncodingEntry {
        name: "UTF-16LE",
        aliases: &["UTF16LE", "utf-16le", "utf16le", "UCS-2LE", "ucs-2le"],
        is_unicode: true,
        decode: |bytes| {
            Utf16Le::validate(bytes)?;
            let mut chars = Vec::new();
            let mut offset = 0;
            while let Some((c, next)) = Utf16Le::decode_char_at(bytes, offset) {
                chars.push(c);
                offset = next;
            }
            Ok(chars)
        ...
        try_encode_char: |c| {
            let mut buf = [0u8; 4];
            let len = Utf16Le::encode_char(c, &mut buf);
            Some(buf[..len].to_vec())
        },
    }
}
...
mod tests {
    ...
    fn test_validate_bmp() {
        // "hello" in UTF-16LE
        let bytes = [0x68, 0x00, 0x65, 0x00, 0x6C, 0x00, 0x6C, 0x00, 0x6F, 0x00];
        assert!(Utf16Le::validate(&bytes).is_ok());
    }

    #[test]
    fn test_validate_odd_length() {
        let bytes = [0x68, 0x00, 0x65];
        assert!(Utf16Le::validate(&bytes).is_err());
    }
    ...
    fn test_validate_lone_surrogate() {
        // Lone high surrogate
        let bytes = [0x3D, 0xD8, 0x68, 0x00];
        assert!(Utf16Le::validate(&bytes).is_err());

        // Lone low surrogate
        let bytes = [0x00, 0xDE, 0x68, 0x00];
        assert!(Utf16Le::validate(&bytes).is_err());
    }

    #[test]
    fn test_decode_bmp() {
        let bytes = [0x68, 0x00]; // 'h'
        let (c, next) = Utf16Le::decode_char_at(&bytes, 0).unwrap();
        assert_eq!(c, 'h');
        assert_eq!(next, 2);
    }
    ...
    fn test_encode_bmp() {
        let mut buf = [0u8; 4];
        let len = Utf16Le::encode_char('h', &mut buf);
        assert_eq!(len, 2);
        assert_eq!(&buf[..2], &[0x68, 0x00]);
    }

    #[test]
    fn test_encode_surrogate_pair() {
        let mut buf = [0u8; 4];
        let len = Utf16Le::encode_char('😀', &mut buf);
        assert_eq!(len, 4);
        assert_eq!(&buf[..4], &[0x3D, 0xD8, 0x00, 0xDE]);
    }

    #[test]
    fn test_is_char_boundary() {
        // "h😀" in UTF-16LE
        let bytes = [0x68, 0x00, 0x3D, 0xD8, 0x00, 0xDE];

        assert!(Utf16Le::is_char_boundary(&bytes, 0));
        assert!(!Utf16Le::is_char_boundary(&bytes, 1)); // Middle of 'h'
        assert!(Utf16Le::is_char_boundary(&bytes, 2)); // Start of emoji
        assert!(!Utf16Le::is_char_boundary(&bytes, 3)); // Middle of high surrogate
        assert!(!Utf16Le::is_char_boundary(&bytes, 4)); // Low surrogate (middle of emoji)
        assert!(!Utf16Le::is_char_boundary(&bytes, 5)); // Middle of low surrogate
        assert!(Utf16Le::is_char_boundary(&bytes, 6)); // End
    }
    ...
    fn test_decode_char_before() {
        ...
        let (c, start) = Utf16Le::decode_char_before(&bytes, 6).unwrap();
        assert_eq!(c, '😀');
        assert_eq!(start, 2);

        let (c, start) = Utf16Le::decode_char_before(&bytes, 2).unwrap();
        assert_eq!(c, 'h');
        assert_eq!(start, 0);
    }
    ...
    fn test_roundtrip_all_bmp() {
        ...
        for cp in 0u32..0x10000 {
            ...
            let len = Utf16Le::encode_char(c, &mut buf);
            let (decoded, _) = Utf16Le::decode_char_at(&buf[..len], 0).unwrap();
